...
## 6. Data Visualization & Formatting

6.1. Sparklines: Use `primary` for growth and `secondary` for stress spikes. Lines should be 1.5px thick with a subtle `primary` glow (5px blur).
6.2. Date & Time Formatting: Standardize all UI timestamps to the following human-readable string: - Month: `short` (e.g., "Jan") - Day: `numeric` - Hour/Minute/Second: `2-digit` - Clock: `12-hour` with `hour12: true` - Example: `Jan 20, 10:45:12 PM` - Implementation utility: `formatDateTime` in `src/utils/format.ts`.

## 7. Do’s and Don’ts

7.1. Do - **Do** use asymmetrical margins (e.g., 20% left, 5% right) for editorial data layouts. - **Do** use `Space Grotesk` for any value that is generated by the machine (IDs, hex codes, time). - **Do** rely on `surface_container_lowest` to create "wells" for content to sit inside. - **Do** Use gap property vs margin for spacing between elements. - **Do** Use `formatDateTime` for all UI timestamps to ensure consistent "Clinical Console" aesthetics.

7.2. Don’t - **Don’t** use a pure black (#000000) background; it destroys the sophisticated slate depth. - **Don’t** use standard Material Design shadows. Use Tonal Layering or Ghost Borders. - **Don’t** use "pill" buttons. Keep corners sharp (`sm` or `md`) to maintain the "Professional Console" feel. - **Don’t** use more than one `primary` action per screen. Use `tertiary` (Ghost) for everything else.
